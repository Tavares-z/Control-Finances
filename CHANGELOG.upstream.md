# Changelog do Upstream (OpenMonetis)

> **Este arquivo é um espelho do CHANGELOG oficial do upstream** (felipegcoutinho/openmonetis).
> Ele existe só como radar do que muda lá a cada release — **pode ser sobrescrito à vontade
> no sync**, não resolva conflito nele. O changelog do *fork* (Control-Finances), com as
> customizações próprias, fica em [`CHANGELOG.md`](./CHANGELOG.md).

Todas as mudanças notáveis do OpenMonetis são documentadas neste arquivo.
