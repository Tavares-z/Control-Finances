...
export async function fetchMonthlySummaryForChat(
  ...
  const totalReceitas = Math.abs(
    toNumber(
      totaisPorTipo.find((r) => r.transactionType === "Receita")?.total,
    ),
  );
  const totalDespesas = Math.abs(
    toNumber(
      totaisPorTipo.find((r) => r.transactionType === "Despesa")?.total,
    ),
  );
  ...
  const porCategoria = await db
    ...
    .where(
      and(commonWhere, eq(transactions.transactionType, "Despesa")),
    )
  ...
  const byCategory: CategorySummary[] = porCategoria
    .map((row) => {
      ...
      return {
        ...
        percentage:
          totalDespesas > 0
            ? Math.round((spent / totalDespesas) * 100)
            : 0,
      };
    })
...
export async function fetchTransactionsForChat(
  userId: string,
  opts: { period?: string; categoryId?: string; limit?: number },
